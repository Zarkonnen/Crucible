...
public class Molten {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < 2; i++) {
			...
			double intensity = i == 0 ? 0.065 : 0.067;//0.07 + Math.sin(i * Math.PI * 2 / 10) * 0.008;
			BufferedImage src = ImageIO.read(new File("/home/zar/Desktop/" + name + ".png"));
			imgs[i] = src;
			...
			for (int y = 0; y < src.getHeight(); y++) {
				...
				for (int x = 0; x < src.getWidth(); x++) {
					...
					if (c == orange || c == darkorange) {
						...
						if (c == orange) {
							if (r.nextInt(maxLightDist) * intensity < Math.pow(closestSq, 0.25)) {
								src.setRGB(x, y, blue);
							}
						} else {
							if (r.nextInt(maxLightDist) * intensity * 0.3 < Math.pow(closestSq, 0.25)) {
								src.setRGB(x, y, blue);
							} else {
								src.setRGB(x, y, orange);
							}
						}
					}
				}
		...
		ImageOutputStream ios = ImageIO.createImageOutputStream(new File("/home/zar/Desktop/" + name + ".gif"));
		GifSequenceWriter gsw = new GifSequenceWriter(ios, BufferedImage.TYPE_INT_RGB, 100, true);
